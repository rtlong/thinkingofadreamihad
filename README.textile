...
* Photo overlay upon hover, revealing whether there is a _click-through link_ on the photo, and/or if there is a high-res version. In the case of there being both, clicking the photo will take the user to the linked resource, while a "Show High Resolution" link will reveal the high-res photo.
* Manipulation of flash objects, in order to fix _Tumblr_'s shitty Flash insertion, via _jQuery_ on document load, in order to add the @wmode=transparent@ param
* "**DISQUS**":disqus comments integration
* Heavy use of _jQuery_, with appropriate, semantic fall-back markup for those without javascript
...
There is also a sample data feature. It uses all the values set in @Hamburglr.rb@ as sample data, for _variables only_, at the moment. I intend to get the blocks working in the future. to use this feature, add @SAMPLE_DATA = true@ to @render.rb@
